...
> Status: **FROZEN**. The fixed-count pairing, ambiguity, retrieve-failure safety, build, and smoke validation gates passed before this protocol was frozen. Freeze reference: `protocol-v1.0`.
...
Final evidence must be generated from the `protocol-v1.0` freeze reference or descendants that do not alter experimental semantics. Any semantic change requires a new protocol version.

## Freeze gate

Freeze validation requirements:
